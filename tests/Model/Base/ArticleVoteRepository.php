 ...
 */
abstract class ArticleVoteRepository extends \Mandango\Repository
{
    /**
     * {@inheritdoc}
     */
    ...
    public function fixMissingReferences($documentsPerBatch = 1000)
    {
        ...
        do {
            ...
            if (count($ids)) {
                $collection = $this->getMandango()->getRepository('Model\Article')->getCollection();
        ...
        do {
            ...
            if (count($ids)) {
                $collection = $this->getMandango()->getRepository('Model\User')->getCollection();
                $referenceCursor = $collection->find(array('_id' => array('$in' => $ids)), array('_id' => 1));
                $referenceIds =  array_values(array_map(function ($result) { return $result['_id']; }, iterator_to_array($referenceCursor)));
