 ...
 */
abstract class ElementQuery extends \Mandango\Query
{
    /**
     * {@inheritdoc}
     */
            ...
            if (isset($data['type'])) {
                if ('textelement' == $data['type']) {
                    if (!isset($identityMaps['textelement'])) {
                        $identityMaps['textelement'] = $mandango->getRepository('Model\TextElement')->getIdentityMap()->allByReference();
                    }
                ...
                if ('formelement' == $data['type']) {
                    if (!isset($identityMaps['formelement'])) {
                        $identityMaps['formelement'] = $mandango->getRepository('Model\FormElement')->getIdentityMap()->allByReference();
                    }
                ...
                if ('textarea' == $data['type']) {
                    if (!isset($identityMaps['textarea'])) {
                        $identityMaps['textarea'] = $mandango->getRepository('Model\TextareaFormElement')->getIdentityMap()->allByReference();
                    }
                ...
                if ('radio' == $data['type']) {
                    if (!isset($identityMaps['radio'])) {
                        $identityMaps['radio'] = $mandango->getRepository('Model\RadioFormElement')->getIdentityMap()->allByReference();
                    }
